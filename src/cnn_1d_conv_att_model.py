...
class Model(object):
    ...
    def build_cnn(self):
        _, end_points = vgg.vgg_19(self.images, num_classes=1000, is_training=self.is_train)
    ...
    def build_rnn(self):
        ...
        for sent_id in range(1, self.max_sentence_num):
            ...
            with tf.variable_scope('attend', reuse=tf.AUTO_REUSE):
                alpha = self.attend(self.visual_feats, sent_feats)
    ...
    def sentence_encode(self, word_embeddings):
        ...
        with tf.variable_scope('sent_decode', reuse=tf.AUTO_REUSE):
            net = tf.layers.conv1d(word_embeddings, filters=sent_feats_dim, kernel_size=3, strides=1)
            sent_feature1 = tf.layers.max_pooling1d(net, pool_size=self.max_caption_length - 2, strides=100)
            net = tf.layers.conv1d(net, filters=sent_feats_dim, kernel_size=3, strides=1)
        ...
        sent_feature1 = tf.reshape(sent_feature1, shape=[self.batch_size, sent_feats_dim])
        sent_feature2 = tf.reshape(sent_feature2, shape=[self.batch_size, sent_feats_dim])
        sent_feature3 = tf.reshape(sent_feature3, shape=[self.batch_size, sent_feats_dim])
    ...
    def attend(self, contexts, output):
        ...
        reshaped_contexts = nn.dropout(reshaped_contexts, self.dropout_rate, self.is_train, name='drop_c')
        output = nn.dropout(output, self.dropout_rate, self.is_train, name='drop_o')
