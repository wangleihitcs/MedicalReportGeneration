...
def coco_caption_metrics(predictions_list, sentences_list, filename_list, vocabulary_path, max_caption_length=100, batch_size=46, is_training=True):
...
def coco_caption_metrics_hier1(predictions_list, sentences_list, filename_list, vocabulary_path, max_caption_length=300, batch_size=64, N=6, M=50, is_training=True):
    with open(vocabulary_path, 'r') as file:
        vocabulary_list = json.load(file)
    word2id = {}
    for i in range(vocabulary_list.__len__()):
    ...
    if not is_training:
        for key in gts.keys():
            ...
            _gts = {1:[str_grundtruth]}
            _res = {1:[str_input]}
            bleu_scorer = Bleu(n=4)
            _bleu, _ = bleu_scorer.compute_score(gts=_gts, res=_res)
            for i in range(4):
                bleu[i] += _bleu[i]
